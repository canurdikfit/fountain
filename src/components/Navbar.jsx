      ...
      >
        <div>
          ...
          <div className={!clicked ? "navbar_sm" : "navbar_lg"}>
            <div className="flex flex-col sm:justify-start justify-between h-full px-5 sm:px-10 py-8">
              <ul className="flex flex-col items-center justify-start gap-4">
                {NavLinks.map((link, key) => (
                  <li
                    key={key}
                    className="text-sm text-[#B6B6B] text-center  w-full font-medium"
                  >
                    <Link
                      className="hover:opacity-100 hover:bg-primary w-full px-4 py-3 rounded-full hover:text-black block"
                      href={link.url}
                    >
                      {link.name}
                    </Link>
